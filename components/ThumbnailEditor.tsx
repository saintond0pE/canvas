...
const ThumbnailEditor: React.FC<ThumbnailEditorProps> = ({ projectId, onNavigateBack }) => {
  ...
  // FIX: Renamed `prompt` to `promptText` to avoid conflict with window.prompt
  ...
  useEffect(() => {
    ...
    setError(null);
    // FIX: Renamed `setPrompt` to `setPromptText`
    setPromptText('');
  }, [projectId]);
  ...
  const handleAIAction = async (action: (base64: string, mimeType: string, prompt?: string) => Promise<string>) => {
    ...
    try {
      // FIX: Renamed `prompt` to `promptText`
      const result = await action(imageToEdit, originalImage.type, promptText);
  ...
  const handleEnhancePrompt = async () => {
      ...
      try {
          // FIX: Renamed `prompt` to `promptText`
          const result = await getPromptCoaching(promptText);
  ...
  const handleApplyCoachingSuggestion = () => {
      if (coachingResult) {
          // FIX: Renamed `setPrompt` to `setPromptText`
          setPromptText(coachingResult.suggestion);
          setCoachingResult(null);
      }
  };
  ...
  return (
    <>
      ...
      `}</style>
      <div className="relative min-h-[calc(100vh-89px)] w-full flex flex-col items-center justify-start">
        <EditorBackgroundShapes />
        
        {/* Top Toolbar */}
        <div className="w-full max-w-7xl mx-auto px-4 md:px-8 py-3 flex items-center justify-between z-10">
            <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
                <button onClick={handleBack} className="flex-shrink-0 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors flex items-center gap-2 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800">
                ...
                <input
                    ...
                    className="w-full sm:w-auto max-w-xs text-lg sm:text-xl font-bold bg-transparent border-0 border-b-2 border-transparent focus:ring-0 focus:border-pink-500 dark:focus:border-pink-400 transition-colors p-1 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 truncate"
                />
            </div>
            <div className="flex items-center gap-2">
                ...
                <div className="relative">
                    ...
                    {isDownloadMenuOpen && (
                        <div className="absolute top-full mt-2 w-48 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-gray-200 dark:border-slate-700 overflow-hidden z-20 right-0">
                ...
                <Button onClick={handleSave} disabled={isLoading} size="sm" className="bg-green-500 hover:bg-green-600 focus:ring-green-500">
        ...
        <div className="w-full flex-1 flex flex-col items-center justify-center px-4 md:px-8">
        ...
        <div className="relative mt-4 mb-8 w-full max-w-4xl px-4 md:px-0">
          <div className="w-full bg-white/50 dark:bg-slate-800/50 backdrop-blur-2xl p-4 rounded-2xl shadow-lg border border-white/50 dark:border-white/10">
              <div className="flex items-center gap-2 sm:gap-4">
                  <div className="relative">
                      <Button 
                          onClick={() => setIsExtensionMenuOpen(!isExtensionMenuOpen)} 
                          variant="icon" 
                          disabled={!editedImage || isLoading} 
                          aria-label="Creative Extensions"
                          title="Creative Extensions"
                      >
                          <MagicWandIcon className="w-6 h-6" />
                      </Button>
                      {isExtensionMenuOpen && (
                          <div className="absolute top-full left-0 mt-2 w-max bg-white/60 dark:bg-slate-800/60 backdrop-blur-2xl p-2 rounded-2xl shadow-lg border border-white/50 dark:border-white/20 flex items-center gap-2 z-20">
                              <Button onClick={handleRemix} variant="icon" disabled={isLoading} title="Remix Style"><RemixIcon className="w-6 h-6" /></Button>
                              <Button onClick={handleInterpretEmotion} variant="icon" disabled={isLoading} title="Interpret Emotion"><EmotionIcon className="w-6 h-6" /></Button>
                              <Button onClick={handleExpand} variant="icon" disabled={isLoading} title="Expand Canvas"><ExpandIcon className="w-6 h-6" /></Button>
                          </div>
                      )}
                  </div>
                  <textarea
                  ...
                  <Button 
                      ...
                      size="md" 
                      className="!py-3 h-full flex-shrink-0"
                  >
                    ...
                    {brainstormIdeas.length > 0 && (
                      <div className="flex flex-wrap justify-center gap-2 animate-fade-in">
                        {brainstormIdeas.map((idea, i) => (
                          // FIX: Renamed `setPrompt` to `setPromptText`
                          <button key={i} onClick={() => setPromptText(idea)} className="px-3 py-1.5 bg-purple-100 dark:bg-purple-800/50 text-purple-800 dark:text-purple-200 rounded-full text-sm font-medium hover:bg-purple-200 dark:hover:bg-purple-800/80 transition-colors shadow-sm hover:shadow-md">
                            "{idea}"
                          </button>
                        ))}
                      </div>
                  ...
                  <>
                    {!coachingResult && !isCoaching && (
                      <div className="text-center animate-fade-in">
                          ...
                          {/* FIX: Renamed `prompt` to `promptText` */}
                          <Button onClick={handleEnhancePrompt} disabled={isLoading || isCoaching || !promptText.trim()} className="bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 focus:ring-pink-500 text-white shadow-lg hover:shadow-pink-500/50">
